...
void main() {
  ...
  ReduxState defaultState = ReduxState(unit: 'lbs', entries: [entry, entry]);
  ...
  pumpSettingWidget(Store<ReduxState> store, WidgetTester tester) async {
    await tester.pumpWidget(StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
      return StoreProvider<ReduxState>(
        store: store,
        child: MaterialApp(home: Scaffold(body: HistoryPage())),
      );
    }));
  }

  testWidgets('HistoryPage has text if there are no entries', (WidgetTester tester) async {
    await pumpSettingWidget(Store<ReduxState>(reduce, initialState: defaultState.copyWith(entries: [])), tester);
  ...
  testWidgets('HistoryPage has 2 items for 2 entries', (WidgetTester tester) async {
    await pumpSettingWidget(Store<ReduxState>(reduce, initialState: defaultState), tester);
    expect(find.byType(WeightListItem), findsNWidgets(2));
  });

  testWidgets('HistoryPage shows snackbar if entry was removed', (WidgetTester tester) async {
    await pumpSettingWidget(
        Store<ReduxState>(
          reduce,
          initialState: defaultState.copyWith(
            removedEntryState: defaultState.removedEntryState.copyWith(hasEntryBeenRemoved: true),
          ),
        ),
        tester);
  ...
  testWidgets('HistoryPage shows snackbar with proper text', (WidgetTester tester) async {
    await pumpSettingWidget(
        Store<ReduxState>(
          reduce,
          initialState: defaultState.copyWith(
  ...
  testWidgets('HistoryPage shows snackbar with proper action', (WidgetTester tester) async {
    await pumpSettingWidget(
        Store<ReduxState>(
          reduce,
          initialState: defaultState.copyWith(
